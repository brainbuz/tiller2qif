...
The C<preview> command is meant to be run after map. It requires running the steps individually (ingest, map, preview, emit).
...
as a starting point and call the sub-modules directly (C<Finance::Tiller2QIF::ReadCSV>,
C<Finance::Tiller2QIF::Map>, C<Finance::Tiller2QIF::WriteQIF>) for finer control.

Note that all functions accept C<db_path> as the database parameter. The CLI
...
Convenience wrapper that calls C<ingest>, C<apply_map>, and C<emit> in sequence
...
Applies category mapping rules to unexported transactions. If C<mapfile> is
omitted, transactions pass through unchanged. C<beforemap> and C<aftermap>
