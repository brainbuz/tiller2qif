...
The repository includes a Visual Studio Code extension, C<vscode-tiller-map/>, which highlights mapping files and preview files and completes destination account names from your chart of accounts. It is not published to the Marketplace and is not part of the CPAN distribution, so it is installed from a checkout of the repository.
...
Mapping files are recognised by the C<.map> and C<.mapping> extensions and by the name C<tiller.mapping>. Preview files are recognised by the C<.t2qpv> extension, which is what C<--viewer> writes, so a preview opened in VS Code is highlighted the same way.

=head2 Chart of Accounts Completion

The destination column of a mapping rule offers C<source>, C<blank>, and C<skip>, and can also offer the real account names from your accounting program. That comes from the extension's settings, not from tiller2qif: set C<tiller2qifMap.coaPath> to a chart-of-accounts export (a leading C<~> is expanded). Without it, completion still offers the three keywords.

C<tiller2qifMap.coaFormat> selects how that file is read.  It defaults to C<auto>, which detects the format; set it explicitly if detection picks the wrong loader. Only the GnuCash chart-of-accounts CSV export is built in today — see C<loaders/index.js> in the extension source for the loader contract if you want to add another program's export.

=head2 Row Colors

C<tiller2qifMap.rowColors> controls row backgrounds in both file types. C<rainbow>, the default, gives rows a repeating series of subtle background colors; C<none> turns backgrounds off. A preview transaction occupies two lines, and both receive the same color.
